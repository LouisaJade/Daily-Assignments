...
```{r}
# Question 1: Create a data.frame of the 5 counties with the most current cases
...
```{r}
# Question 2: Create a data.frame of the 5 states with the most current cases
top_5_states <- covid %>%
  filter(date == max(date)) %>%        
  group_by(state) %>%                  
...
```{r}
# Question 3: Report the 5 counties with the worst current death/cases ratio
worst_ratio_counties <- covid %>%
  filter(date == max(date)) %>%            
...
```{r}
# Question 4: Report the 5 states with the worst current death/cases ratio
worst_ratio_states <- covid %>%
  filter(date == max(date)) %>%           
  group_by(state) %>%                      
  summarize(total_cases = sum(cases), total_deaths = sum(deaths)) %>%  
  mutate(death_case_ratio = (total_deaths / total_cases) * 100) %>%  
  arrange(desc(death_case_ratio)) %>%      
  slice(1:5)                              
